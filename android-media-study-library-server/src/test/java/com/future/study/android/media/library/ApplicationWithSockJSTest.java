...
@SpringBootTest(
        classes={ApplicationWithSockJS.class},
        webEnvironment = SpringBootTest.WebEnvironment.DEFINED_PORT)
public class ApplicationWithSockJSTest {
    private final static Logger logger=LoggerFactory.getLogger(ApplicationWithSockJSTest.class);
    ...
    @Test
    public void simulateTestingVoiceDataTransfer() throws InterruptedException {
        WebSocketStompClient stompClient2=this.getStompClient("1-2",new StompSessionHandlerAdapter(){
            @Override
            public void afterConnected(StompSession session, StompHeaders connectedHeaders) {
                session.subscribe("/user/queue/receiveVoiceData", new StompSessionHandlerAdapter() {
                    @Override
                    public void handleFrame(StompHeaders headers, Object payload) {
                        logger.info("收到消息["+payload+"]");
                        session.send("/app/sendVoiceData", "你");
                    }
                });
            }
        ...
        WebSocketStompClient stompClient1=this.getStompClient("1-1",new StompSessionHandlerAdapter(){
            ...
            public void afterConnected(StompSession session, StompHeaders connectedHeaders) {
                ...
                session.send("/app/sendVoiceData", "我");
            }
