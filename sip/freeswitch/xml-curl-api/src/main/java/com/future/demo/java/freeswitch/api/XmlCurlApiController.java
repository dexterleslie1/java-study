...
public class XmlCurlApiController {
    ...
    */
   @RequestMapping(value = "curl", produces = MediaType.TEXT_PLAIN_VALUE)
   public ResponseEntity<String> curl(HttpServletRequest request) throws ServletRequestBindingException {
	   	String callee = ServletRequestUtils.getStringParameter(request, "callee");
   ...
   /**
    * 转换request parameterMap到字符串
    * @param request
    * @param isNewline 参数之间是否新换行
    * @return
    */
   public static StringBuilder getParameterMapAsString(HttpServletRequest request, boolean isNewline) {
       StringBuilder builder = new StringBuilder();
       builder.append("Dialplan parameters(");
       Map<String, String[]> mapParams = request.getParameterMap();
       Set<String> keys = mapParams.keySet();
       int counter = 0;
       for(String key : keys) {
           String []values = mapParams.get(key);
           String keyValueString = String.format("%s=%s", key, (values==null?null:String.join(",", values)));
           builder.append(keyValueString);
           if(!isNewline) {
               if (counter + 1 < keys.size()) {
                   builder.append("; ");
               }
           } else {
               builder.append("\r\n");
...
//           }
    ...
    public ResponseEntity<String> dialplan(HttpServletRequest request) throws ServletRequestBindingException {
    	StringBuilder builder = getParameterMapAsString(request, true);
 	   	System.out.println(builder);
 	   	String caller = ServletRequestUtils.getStringParameter(request, "Caller-Caller-ID-Number");
...
//				"    			<action application=\"curl\" data=\"http://23.91.97.126:44795/api/v1/freeswitch/curl?caller="+caller+"&callee="+callee+"\"/>\n" +
...
//    			"</document>";
 	   ...
 	   String dialplan = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n" + 
   			...
   			"	<context name=\"public\">\n" +
   			"		<extension name=\"public_extensions\">\n" +
   			"	  		<condition field=\"destination_number\" expression=\"\\+12058801417$\">\n" +
				"        		<action application=\"ring_ready\"/>\n" + 
				"    			<action application=\"curl\" data=\"http://23.91.97.126:44795/api/v1/freeswitch/curl?caller="+caller+"&callee="+1001+"\"/>\n" + 
